// lib/db.ts — Generic Supabase fetch functions for the dynamic course system.
// These currently use mock data. When your Supabase tables are ready,
// swap the mock logic for real `supabase.from('lessons').select(...)` calls.
...
const mockLessons: Lesson[] = [
  ...
  <div class="mt-16 pt-10 border-t border-gray-800 border-dashed">
    <h3 class="text-3xl font-extrabold text-white mb-6">🚨 Deep Dive: The $10,000 Mistake</h3>
    <div class="prose prose-invert prose-lg max-w-none prose-p:text-gray-400">
      <p>Let’s talk about real-world stakes. In 2023, a Nigerian logistics startup lost exactly ₦8.5 million in a single week because of a misunderstood API integration email sent by their payment gateway. The email was loaded with heavy technical jargon ("Webhooks," "Asynchronous Callbacks," "Idempotency Keys"). The founder, not wanting to look incompetent, skimmed the email, assumed it was a standard update, and forwarded it to a junior dev with a simple "Please implement."</p>
      <p>The system failed silently. Customers were paying, but the database wasn’t recording it. Goods were being dispatched for free.</p>
      <p>If that founder had taken 30 seconds to snap a screenshot of that email and drop it into their AI with the <strong>S.U.C. Framework</strong> ("Look at this email. Explain what it means for my checkout flow in simple English, and tell me exactly what my developer needs to do to avoid downtime"), the AI would have instantly translated the jargon into a flashing red warning: <em>"Your current payment system will break on Friday unless your developer changes the callback URL."</em></p>
      
      <h4>The Cost of Pride</h4>
      <p>We often refuse to ask for help because we fear sounding stupid. The beauty of Artificial Intelligence is that it has no ego to bruise. It doesn't judge you. It doesn't gossip. It doesn't screenshot your questions and post them on Twitter.</p>
      <div class="bg-gray-800/50 p-6 rounded-xl border border-gray-700 my-8 italic text-cyan-100/80">
        "Your AI is your private, infinite-patience tutor. You can ask it to explain the difference between a debit and a credit 50 times in a row, and it will calmly explain it using a different market analogy every single time."
      </div>
      
      <h4>Case Study 2: The Tailor and the Invoice</h4>
      <p>A fashion entrepreneur in Lekki secured a massive contract to design wedding outfits for a family of 30. The client sent a highly complex PDF containing different fabric requests, measurements, delivery timelines spread across three months, and strict penalties for late deliveries.</p>
      <p>Instead of manually writing down the dates and calculating the risks, the tailor uploaded the PDF to the AI.</p>
      <p><strong>The Command:</strong> <em>"Read this contract. I am the tailor. Extract a clear, chronologically ordered to-do list for exactly what I need to sew and deliver each week. Also, point out any penalty clauses I need to be worried about."</em></p>
      <p><strong>The Result:</strong> The AI caught a hidden clause stating that any outfit delivered after 4 PM on a Friday incurred a 10% discount penalty. The tailor adjusted her logistics immediately, saving hundreds of thousands of Naira.</p>

      <h3 class="text-2xl font-bold text-white mt-12 mb-4">Advanced Tactics: Modifying Images</h3>
      <p>What if the problem isn’t the whole image, but just a tiny part of it? If you upload a screenshot of your entire computer desktop, the AI gets distracted by all the icons, the clock, and the browser tabs. This is called "noise".</p>
      <ul class="space-y-4">
        <li><strong>The Crop Rule:</strong> Always crop the image before uploading. If you are asking about an error message, crop the image so ONLY the error box is visible. The smaller the haystack, the faster the AI finds the needle.</li>
        <li><strong>The Circle Method:</strong> Use your phone's default photo editor to literally draw a red circle or an arrow pointing to the specific button or text you are asking about. The AI's vision engine prioritizes items that are highlighted or circled.</li>
        <li><strong>The Sequence:</strong> If you are asking about a multi-step problem (e.g., "I clicked this button, and then this screen popped up, and then it froze"), upload the images in order, and label them in your prompt: "Image 1 shows the start. Image 2 shows the error."</li>
      </ul>
    </div>
  </div>

  <div class="mt-16 pt-10 border-t border-gray-800 border-dashed pb-20">
    <h3 class="text-3xl font-extrabold text-white mb-6">Summary Cheat Sheet</h3>
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      <div class="bg-black/40 p-5 rounded-lg border border-gray-800">
        ...
        <ul class="text-sm text-gray-400 space-y-2 list-disc pl-4">
          <li>Crop out irrelevant background noise from screenshots.</li>
          <li>Use red arrows or circles to guide the AI's eye.</li>
          <li>Add a highly specific text command alongside the image.</li>
        </ul>
      </div>
      <div class="bg-black/40 p-5 rounded-lg border border-gray-800">
        <strong class="text-red-400 block mb-2">DON'T:</strong>
        <ul class="text-sm text-gray-400 space-y-2 list-disc pl-4">
          <li>Never upload unredacted bank statements or BVNs.</li>
          <li>Never upload blurry, dark, or out-of-focus photos.</li>
          <li>Never upload an image without a text prompt.</li>
        </ul>
      </div>
    </div>
  </div>
 ...
 * Replace with: supabase.from('lessons').select('*').eq('id', lessonId).single()
 */
export async function getLesson(moduleId: string, lessonId: string): Promise<Lesson | null> {
  const lesson = mockLessons.find(l => l.id === lessonId && l.moduleId === moduleId);
  return lesson || null;
}

/**
 * Fetches all lessons belonging to a module, ordered by their position.
 * Replace with: supabase.from('lessons').select('*').eq('module_id', moduleId).order('order')
 */
